...
Created: June 2016
Version: 1.3.0
# Added Annotation Notes in Table format

.NOTES
Author: Aaron Timko
Created: August 2016
Version: 1.3.1
# Added Socket and TotalvCPU logic
...
CSV File - VM info with the following headers
    Name, Boot, OSType, Template, CustSpec, Folder, ResourcePool, CPU, RAM, Disk2, Disk3, Disk4, Datastore, DiskStorageFormat, NetType, Network, DHCP, IPAddress, SubnetMask, Gateway, pDNS, sDNS, tDNS, qDNS, Notes
...
CSV Field Definitions
	...
	tDNS - Tertiary DNS (Windows Only)
	qDNS - Quaternary DNS (Windows Only)
...
$headers = "" | Select-Object Name, Boot, OSType, Template, CustSpec, Folder, ResourcePool, CPU, RAM, Disk2, Disk3, Disk4, Datastore, DiskStorageFormat, NetType, Network, DHCP, IPAddress, SubnetMask, Gateway, pDNS, sDNS, tDNS, qDNS, Notes, jira, created, owner
$taskTab = @{}
 
# ISE Test Variables

#$csvfile = "D:\Scripts\VMDeploy_v2\DeployVM_LinuxExample_1.3.csv"
#$vcenter = "vcenter.keynote.com"
...
If ($csvfile -eq "" -or !(Test-Path $csvfile) -or !$csvfile.EndsWith(".csv")) {
...
Foreach ($VM in $newVMs) {
    ...
    If ($vm.DHCP -match "true") {
		...
		If ($VM.OSType -eq "Windows") {
	        ...
	        -Dns $VM.pDNS,$VM.sDNS,$VM.tDNS,$VM.qDNS -DefaultGateway $VM.Gateway | Out-Null
    ...
    #$taskTab
    $taskTab[(New-VM -Name $VM.Name -ResourcePool $VM.ResourcePool -Location $VM.Folder -Datastore $VM.Datastore `
...
#      $NumCoresPerSocket = $VMconfig.TotalvCPU / $VMconfig.Sockets
#      $cpu_spec=New-Object –Type VMware.Vim.VirtualMachineConfigSpec –Property @{“NumCoresPerSocket” = $NumCoresPerSocket}
#      ($VM).ExtensionData.ReconfigVM_Task($cpu_spec)
      ...
      #Set owner, creation date, and JIRA ticket
      Out-Log "Setting Annotations and Notes on $vmName" "Yellow"
	  ForEach ($row in $newVMs) {
        $MyCol=@()
        
		## get the VM for this row
		$result = Set-Annotation -Entity $VM -CustomAttribute "JIRA" -Value $row.jira
            $notesObj = New-Object PSCustomObject
            $notesObj | Add-Member -Type NoteProperty -name "Server" -value $result.AnnotatedEntity.Name
            $notesObj | Add-Member -Type NoteProperty -name "Annotation" -value $result.Name
            $notesObj | Add-Member -Type NoteProperty -name "Value" -value $result.Value
            $MyCol += $notesObj
		$result = Set-Annotation -Entity $VM -CustomAttribute "Created" -Value $row.created
            $notesObj = New-Object PSCustomObject
            $notesObj | Add-Member -Type NoteProperty -name "Server" -value $result.AnnotatedEntity.Name
            $notesObj | Add-Member -Type NoteProperty -name "Annotation" -value $result.Name
            $notesObj | Add-Member -Type NoteProperty -name "Value" -value $result.Value
            $MyCol += $notesObj
		$result = Set-Annotation -Entity $VM -CustomAttribute "Owner" -Value $row.owner
            $notesObj = New-Object PSCustomObject
            $notesObj | Add-Member -Type NoteProperty -name "Server" -value $result.AnnotatedEntity.Name
            $notesObj | Add-Member -Type NoteProperty -name "Annotation" -value $result.Name
            $notesObj | Add-Member -Type NoteProperty -name "Value" -value $result.Value
            $MyCol += $notesObj
		$result = Set-Annotation -Entity $VM -CustomAttribute "Project" -Value $row.project
            $notesObj = New-Object PSCustomObject
            $notesObj | Add-Member -Type NoteProperty -name "Server" -value $result.AnnotatedEntity.Name
            $notesObj | Add-Member -Type NoteProperty -name "Annotation" -value $result.Name
            $notesObj | Add-Member -Type NoteProperty -name "Value" -value $result.Value
